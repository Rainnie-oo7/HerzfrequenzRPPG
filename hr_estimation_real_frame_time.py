...
# Video einlesen, Return check, Framesetzen
...
# Polygon aus gespeicherten Videos, oder neues Video neues Polygon
polygon_points = []
antwort = input(f"Neues Polygon für '{VIDEO_FILENAME}' zeichnen? (j/n): ").strip().lower()
# Benutzer kann neues Polygon wählen,anklicken
if antwort == "j":
    ...
    while True:
        ...
        if key == ord("y") and len(polygon_points) >= 3:
            ...
            exit()
    cv2.destroyAllWindows()
else:   # Checksum, ob polygon für bekanntes Video gespeichert vorliegt
...
polygon = np.array(polygon_points, dtype=np.int32)  # Polygonpunkte int32 Konvertieren
mask = np.zeros(frame.shape[:2], dtype=np.uint8)    # Maske Schablone leer
cv2.fillPoly(mask, [polygon], 255)        # infill255 was Polygon auf leere Maske darauf

# Butterworth mit omega Grenzfrequenzen
def bandpass_filter(signal, fps, low=0.7, high=3.0):
    nyq = 0.5 * fps #Nyquist minimale Länge
    b, a = butter(2, [low / nyq, high / nyq], btype='band')
    return filtfilt(b, a, signal)

# nicht verwendet, Fensterung und Glättung
...
# Gemittelte Intensitäten n-frames zusammen
def moving_average(signal, n):
    return np.convolve(signal, np.ones(n) / n, mode='valid')

# Samplingrate, Zeitreihen, Intensitätsreihen
...
# Aktuelle Quarzzeit, globale Variablen Init für Intervalle-Taktung
start_time = time.time()
last_10s_time = start_time
last_1s_time = start_time

# letztes Frame = Videoende Init, Plot hält wenn Video hält
...
while cap.isOpened():
    ...
    roi = cv2.bitwise_and(frame, frame, mask=mask)  #ROI=Polygon 1 0 wird zu 1
    green_mean = np.mean(roi[:, :, 1][mask == 255]) #Mittlerung grüne Intensitätswerte
    now = time.time()                               #Ausgleich Laufzeit Programm |! funktioniert nicht immer
    current_time = now - start_time                 #Aktuelle Zeit Startreferenz

    intensity_buffer.append(green_mean)
    time_buffer.append(current_time)

    times = np.array(time_buffer)                   #List->Array Konvertion
    signal = np.array(intensity_buffer)

    bpm_estimate = None                             #HR-Variable Init
    if len(signal) >= sampling_rate * 5:            #Nyquist-Länge = 5 Sekunden
        if method == "roh":
            minima, _ = find_peaks(-signal, distance=sampling_rate * 0.5)
            duration = len(signal) / sampling_rate      # FPS
            bpm_estimate = len(minima) / duration * 60  # Gibt bestenfalls pro Frame BPM aus
            ax2.clear()
            ...
            N = len(signal)         #Anzahl der Messpunkte wie viele Samples im aktuellen Buffer enthalten sind
            T = 1 / sampling_rate   #Zeitintervall zwischen zwei Frames (in Sekunden),T = 1/30 = 0,033 s
            yf = fft(signal - np.mean(signal)) # Ergebnis ist ein komplexes Spektrum.Mittelwert abgezogen, um Gleichanteile (DC-Offset) zu entfernen.
            xf = fftfreq(N, T)[:N // 2] # Erzeugung das Frequenzspektrum in Hertz für die positiven Frequenzen.([:N // 2])
            bpm = xf * 60                           # Umrechnung Hz in bpm
            amplitude = 2.0 / N * np.abs(yf[0:N // 2]) # Berechnung normierter Amplituden des Freq.spektrums (für positive Frequenzen).
            # Dadurch werden uninteressante Frequenzen herausgefiltert.
            valid = (bpm >= 72) & (bpm <= 165)         # Begrenzung

            if np.any(valid):
                bpm_estimate = bpm[valid][np.argmax(amplitude[valid])] # Der Suche den Index dazugehörige Maximum bpm-Wert ist unsere Herzfrequenzschätzung (Peak). stärkster Frequenzanteil. Nicht immer richtig.
                line2.set_data(bpm[valid], amplitude[valid])
                ax2.set_ylim(0, np.max(amplitude[valid]) * 1.1)     #Plotting
    ...
    # Output Herzfrequnz alle 10 Sekunden
    if bpm_estimate and 72 <= bpm_estimate <= 165:
        ...
        if now - last_1s_time >= 1:
            print(f"[{int(current_time)}s] BPM (1s): {bpm_estimate:.1f}")
...
# Output Herzfrequnz alle 1 Sekunde
